...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function applyTheme(theme) {
        ...
        const splitOffset = 4;
        const selectedDate =
            (currentColumnIndex >= 0 && currentColumnIndex < dateCells.length ? dateCells[currentColumnIndex]?.textContent.trim() : "") ||
            (currentColumnIndex >= splitOffset ? splitDateCells[currentColumnIndex - splitOffset]?.textContent.trim() : "");
        if (selectedDate) {
  ...
  <button type="button" id="whatsnew-ok" style="padding:8px 12px; background: var(--button-color); color: var(--text-strong-color); border: 1px solid var(--border-color); border-radius: 8px; cursor: pointer;">OK</button>
    ...
    function selectColumn(index) {
        ...
        if (useSplit) {
            ...
            const localIndex = index - splitOffset;
    ...
    function updateFromJSON(dateText) {
        ...
        } else if (timetableSplit && splitDateCells.length > 0 && currentColumnIndex >= splitOffset) {
            const localIndex = currentColumnIndex - splitOffset;
            const splitRows = timetableSplit.querySelectorAll("tbody tr");
            splitRows.forEach((row, rowIndex) => {
    ...
    splitSubjectCells.forEach(cell => {
        cell.addEventListener("click", () => {
            ...
            const globalColIndex = splitOffset + localColIndex;
    ...
    splitDateCells.forEach((cell, index) => {
        cell.addEventListener("click", () => {
            const splitOffset = 4;
            selectColumn(splitOffset + index);
